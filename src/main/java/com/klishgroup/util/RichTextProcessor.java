...
public final class RichTextProcessor {
    ...
    public static transient Logger LOGGER = Logger.getLogger(RichTextProcessor.class);

    public static final int LOCKED_VIEW_MAX_DISPLAY_CHARACTERS = 1500;
    ...
    private boolean renderLockedView;

    private transient int characterCount;
    ...
    public RichTextProcessor renderLockedView(boolean renderLockedView) {
        this.renderLockedView = renderLockedView;
        return this;
    }
    ...
    private List<RteNode> toRteNodes(List<Node> siblings, Map<String, ObjectType> tagTypes) {
        ...
        for (Node sibling : siblings) {
            ...
            if (sibling instanceof Element && !hideNode()) {
            ...
            } else if ((sibling instanceof TextNode || sibling instanceof DataNode) && !hideNode()) {

                if (sibling instanceof TextNode) {
                    String siblingText = ((TextNode) sibling).text();

                    if (renderLockedView) {
                        int maxDisplayLengthLeft = LOCKED_VIEW_MAX_DISPLAY_CHARACTERS - characterCount;
                        // only need split the string if it is longer than the number of remaining allowed display characters
                        if (siblingText.length() > maxDisplayLengthLeft) {
                            siblingText = siblingText.substring(0, maxDisplayLengthLeft);
                        }
                        characterCount += siblingText.length();
                    }

                    rteNodes.add(new RteHtml(siblingText));
    ...
    private boolean hideNode() {
        ...
        return characterCount >= LOCKED_VIEW_MAX_DISPLAY_CHARACTERS ? true : false;
    }
    ...
    public static class LineBreakProcessor implements RichTextProcessorFunction {
        ...
        public String apply(String html) {
            ...
            if (!StringUtils.isBlank(rteTagsDomSelector)) {
                for (Element enhancement : body.select(getRteTagsDomSelector())) {
